...
`layout` - «сетка» или «раскладка». Классы с этим префиксом (например, `layout-column`) предназначены для создания сетки. Обычно для таких классов задают только флоаты, размеры и внешние отступы.
...
`allow` - разрешить (подписку на новости, использование персональных данных и т.д.)
